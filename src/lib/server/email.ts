...
// there.
//
// Sends from the shared `onboarding@resend.dev` sandbox sender — no domain
// verification needed, but Resend restricts that sender to only mail the
// account's own signup address. Fine for a single-user deployment; if more
// users are added later, verify a domain in Resend (or switch providers).
